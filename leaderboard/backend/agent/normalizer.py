...
def classify_scope(lb_id: int, db: Session):
    """
    Lightweight scope classification using already-stored text — no URL fetch.
    Only runs if lb.scope is still NULL. Asks Gemini with the stored description
    + methodology as context.
    """
    ...
    if not context:
        # No stored text — fall back to fetching the URL
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if data.get("scope") in ("Global", "Regional"):
                lb.scope = data["scope"]
                db.commit()
                print(f"  Scope classified: {lb.name} → {lb.scope}")
    except Exception as e:
