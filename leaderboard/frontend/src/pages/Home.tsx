...
function countFor(cat: DomainCategory, leaderboards: Leaderboard[]): number {
...
export default function Home() {
  ...
  const [leaderboards, setLeaderboards] = useState<Leaderboard[]>(
    () => getCached<Leaderboard[]>("/leaderboards") ?? []
  );

  useEffect(() => {
    api.listDomainCategories().then(setCategories);
    api.listLeaderboards().then(setLeaderboards);
  }, []);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {categories.map((cat) => {
          ...
          const count = countFor(cat, leaderboards);
          return (
            <button
              key={cat.slug}
