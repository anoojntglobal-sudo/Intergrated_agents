...
def posts(
    ...
    params: list = []
    if tier:
        where.append(f"classification_class IN ({', '.join('?' for _ in tier)})")
        params.extend(tier)
    if category:
        where.append("matched_category = ?")
...
def runs(
    ...
    rows = db.query(
        ...
        "FROM linkedin_runs ORDER BY started_at DESC LIMIT ?",
        (limit,),
    )
...
def run_now(
    ...
    if db.query(
        ...
        "AND started_at > datetime('now', '-1 hour') LIMIT 1"
    ):
