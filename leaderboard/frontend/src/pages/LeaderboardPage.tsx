...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    api.getScanLogs(lbId).then(setScanLogs);

    api.getLeaderboard(lbId)
      .then((data) => {
        setLb(data);
        const findCat = (cats: DomainCategory[]) =>
          cats.find((c) =>
        ...
        if (cached) {
          setAdminCat(findCat(cached));
        } else {
          api.listDomainCategories().then((cats) => setAdminCat(findCat(cats)));
        }
  ...
  const colOrder: string[] = lb?.column_order && lb.column_order.length > 0 ? lb.column_order : [];
