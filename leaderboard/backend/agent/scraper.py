...
def _wait_for_table(page, timeout_ms=14000):
    # Wait until a real leaderboard table (>= 3 columns, >= 3 rows) is present.
    # A plain wait_for_selector("table") returns immediately when keyboard-shortcut
    # tables are already in the DOM, so we poll for a substantive table instead.
    # Gradio uses SSE; allow 14 s for the server to respond and update the DOM.
    try:
...
def try_tabs(page, deadline):
    ...
    # JS fallback: find ANY visible element with short leaderboard-keyword text and click it
    try:
...
with sync_playwright() as p:
    ...
    # 60 s overall pagination budget (show-all + pagination handled inside collect_all_rows)
...
def _parse_generic(url: str, lb_name: str = "") -> tuple[list, int]:
    ...
    print(f"  Playwright fallback for {lb_name or url}")
    ...
    # Fires when DOM strategies return 0 rows but the page loaded (text exists).
    # Passes visible page text — never Gemini's training knowledge.
    if OPENROUTER_API_KEY and check_html:
        print(f"  Gemini text fallback for {lb_name or url}")
        rows = _gemini_text_extract(check_html, url)
        if rows:
            return rows, combined_status

    # ── Step 4: Gemini vision ─────────────────────────────────────────────
    # Final resort: all DOM/text strategies returned 0 rows but the page loaded.
    # Try visual extraction whenever we have a screenshot — Gemini returns []
    # for non-chart pages so there is no false-positive risk.
    if OPENROUTER_API_KEY and screenshot_b64:
        print(f"  Gemini vision fallback for {lb_name or url}")
        rows = _gemini_visual_extract(screenshot_b64, url)
        if rows:
            return rows, combined_status

    print(f"  All strategies exhausted for {lb_name or url}")
...
def _parse_hf_space(url: str) -> tuple[list, int]:
    """
    Convert huggingface.co/spaces/owner/space → owner-space.hf.space (lowercase, hyphens).
    Then run the full cascade: Playwright → Gemini text → Gemini vision.
    """
    space_url = url
    if "huggingface.co/spaces/" in url:
        ...
        if len(parts) >= 2:
            owner = parts[0].lower()
            # HF subdomain: lowercase, underscores → hyphens
